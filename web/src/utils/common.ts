...
import { formatWithTimezone } from './dayjs'
...
export const checkFileMimetype = (fileName: string, fileType: string) => {
    ...
    // 特殊处理 ico 文件
    if (fileSuffix === 'ico' || fileType === 'image/x-icon' || fileType === 'image/vnd.microsoft.icon') {
        return true
    }
...
export const timeFormat = (dateTime: string | number | null = null, fmt = 'yyyy-mm-dd hh:MM:ss') => {
    ...
    let newFmt = fmt
        .replace(/yyyy/g, 'YYYY')
        .replace(/MM/g, '__MINUTE__')  // 分钟，使用临时占位符
        .replace(/mm/g, 'MM')  // 月份
        .replace(/dd/g, 'DD')
        .replace(/hh/g, 'HH')
        .replace(/ss/g, 'ss')
        .replace(/__MINUTE__/g, 'mm')  // 恢复分钟占位符
    
    return formatWithTimezone(dateTime, newFmt)
}
