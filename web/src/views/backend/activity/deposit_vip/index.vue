<template>
    <div class="activity-config-container">
        <ContentWrap title="530活动配置" v-loading="loading">
            <div class="config-content">
                <el-form ref="formRef" :model="formData" :rules="rules" label-width="120px" @submit.prevent status-icon class="config-form">
                    ...
                    <div class="vip-levels">
                        <h3 class="vip-title">VIP等级配置</h3>

                        <!-- VIP1配置 -->
                        <div class="vip-level-container">
                            <h4>VIP1 配置</h4>

                            <el-form-item label="VIP1奖金总额">
                                <el-input v-model.number="vipFields.vip1.bonus_sum" placeholder="例如: 200" class="vip-input" type="number">
                                    <template #append>%</template>
                                </el-input>
                            </el-form-item>

                            <el-form-item label="VIP1投注次数奖金">
                                <el-input v-model.number="vipFields.vip1.bet_num_bonus" placeholder="例如: 50" class="vip-input" type="number">
                                    <template #append>%</template>
                                </el-input>
                            </el-form-item>

                            <el-form-item label="VIP1投注金额奖金">
                                <el-input v-model.number="vipFields.vip1.bet_money_bonus" placeholder="例如: 30" class="vip-input" type="number">
                                    <template #append>%</template>
                                </el-input>
                            </el-form-item>
                        </div>
                        ...
                        <div class="vip-level-container">
                            ...
                            <el-form-item label="VIP2奖金总额">
                                <el-input v-model.number="vipFields.vip2.bonus_sum" placeholder="例如: 300" class="vip-input" type="number">
                                    <template #append>%</template>
                                </el-input>
                            </el-form-item>

                            <el-form-item label="VIP2投注次数奖金">
                                <el-input v-model.number="vipFields.vip2.bet_num_bonus" placeholder="例如: 80" class="vip-input" type="number">
                                    <template #append>%</template>
                                </el-input>
                            </el-form-item>

                            <el-form-item label="VIP2投注金额奖金">
                                <el-input v-model.number="vipFields.vip2.bet_money_bonus" placeholder="例如: 50" class="vip-input" type="number">
                                    <template #append>%</template>
                                </el-input>
                            </el-form-item>
                        </div>
                        ...
                        <div class="vip-level-container">
                            ...
                            <el-form-item label="VIP3奖金总额">
                                <el-input v-model.number="vipFields.vip3.bonus_sum" placeholder="例如: 500" class="vip-input" type="number">
                                    <template #append>%</template>
                                </el-input>
                            </el-form-item>

                            <el-form-item label="VIP3投注次数奖金">
                                <el-input v-model.number="vipFields.vip3.bet_num_bonus" placeholder="例如: 120" class="vip-input" type="number">
                                    <template #append>%</template>
                                </el-input>
                            </el-form-item>

                            <el-form-item label="VIP3投注金额奖金">
                                <el-input v-model.number="vipFields.vip3.bet_money_bonus" placeholder="例如: 80" class="vip-input" type="number">
                                    <template #append>%</template>
                                </el-input>
                            </el-form-item>
                        </div>
                ...
                <div class="json-preview">
                    ...
                    <div class="json-content">
                        ...
                        <div class="json-section">
                            <h5>VIP配置:</h5>
                            <pre>{{ prettyPrintJSON(vipFields) }}</pre>
                        </div>
                    </div>
                </div>
            </div>
...
interface VipLevel {
    bonus_sum: number
    bet_num_bonus: number
    bet_money_bonus: number
}

// API初始化
const api = new baTableApi('/admin/activity.deposit_vip/')
const formRef = ref<FormInstance>()
const submitting = ref(false)
const loading = ref(false)

// 表单数据
const formData = reactive<VipConfig>({
    id: 1,
    title: '',
    context: '',
    enable_reward: 0,
...
const vipFields = reactive<Record<string, VipLevel>>({
    vip1: { bonus_sum: 0, bet_num_bonus: 0, bet_money_bonus: 0 },
    vip2: { bonus_sum: 0, bet_num_bonus: 0, bet_money_bonus: 0 },
    vip3: { bonus_sum: 0, bet_num_bonus: 0, bet_money_bonus: 0 },
})
...
// 处理VIP输入（简化版，无验证）
const handleVipInput = (vipLevel: 'vip1' | 'vip2' | 'vip3', field: keyof VipLevel) => {
    let value = vipFields[vipLevel][field]
    ...
    vipFields[vipLevel][field] = value
    formData[vipLevel] = JSON.stringify(vipFields[vipLevel])
}
...
watch(
    ...
    () => {
        try {
            const parsed = JSON.parse(formData.vip1 || '{}')
            vipFields.vip1 = {
                bonus_sum: Number(parsed.bonus_sum) || 0,
                bet_num_bonus: Number(parsed.bet_num_bonus) || 0,
                bet_money_bonus: Number(parsed.bet_money_bonus) || 0,
            }
        } catch {
            vipFields.vip1 = { bonus_sum: 0, bet_num_bonus: 0, bet_money_bonus: 0 }
        }
    },
    { immediate: true }
)
...
watch(
    ...
    () => {
        try {
            const parsed = JSON.parse(formData.vip2 || '{}')
            vipFields.vip2 = {
                bonus_sum: Number(parsed.bonus_sum) || 0,
                bet_num_bonus: Number(parsed.bet_num_bonus) || 0,
                bet_money_bonus: Number(parsed.bet_money_bonus) || 0,
            ...
            vipFields.vip2 = { bonus_sum: 0, bet_num_bonus: 0, bet_money_bonus: 0 }
        }
    },
    { immediate: true }
)
...
watch(
    ...
    () => {
        try {
            const parsed = JSON.parse(formData.vip3 || '{}')
            vipFields.vip3 = {
                bonus_sum: Number(parsed.bonus_sum) || 0,
                bet_num_bonus: Number(parsed.bet_num_bonus) || 0,
                bet_money_bonus: Number(parsed.bet_money_bonus) || 0,
            ...
            vipFields.vip3 = { bonus_sum: 0, bet_num_bonus: 0, bet_money_bonus: 0 }
        }
    },
    { immediate: true }
)
...
watch(
    () => vipFields.vip1,
    () => {
        formData.vip1 = JSON.stringify(vipFields.vip1, null, 2)
    },
    { deep: true }
)
...
watch(
    () => vipFields.vip2,
    () => {
        formData.vip2 = JSON.stringify(vipFields.vip2, null, 2)
    },
    { deep: true }
)
...
watch(
    () => vipFields.vip3,
    () => {
        formData.vip3 = JSON.stringify(vipFields.vip3, null, 2)
    },
    { deep: true }
)
...
const submit = async () => {
    try {
        // 手动更新所有VIP字段到formData
        ;['vip1', 'vip2', 'vip3'].forEach((vip) => {
            formData[vip] = JSON.stringify(vipFields[vip])
        })
        ...
        const postData = {
            ...
            vip1: JSON.stringify(vipFields.vip1),
            vip2: JSON.stringify(vipFields.vip2),
            vip3: JSON.stringify(vipFields.vip3),
...
const getInfo = async () => {
    ...
    try {
        ...
        if (res.code === 1) {
            ...
            // 处理VIP配置
            const parseVipConfig = (vipData: any, level: string) => {
                try {
                    const data = typeof vipData === 'string' ? JSON.parse(vipData) : vipData
                    vipFields[level] = {
                        bonus_sum: Number(data.bonus_sum) || 0,
                        bet_num_bonus: Number(data.bet_num_bonus) || 0,
                        bet_money_bonus: Number(data.bet_money_bonus) || 0,
                    }
                    formData[level] = JSON.stringify(vipFields[level])
                } catch (e) {
                    console.error(`处理VIP${level}数据出错:`, e)
                    vipFields[level] = { bonus_sum: 0, bet_num_bonus: 0, bet_money_bonus: 0 }
                    formData[level] = '{}'
                }
            }

            // 处理各VIP等级数据
            parseVipConfig(row.vip1, 'vip1')
            parseVipConfig(row.vip2, 'vip2')
            parseVipConfig(row.vip3, 'vip3')
        } else {
