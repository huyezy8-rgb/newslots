<template>
    <div class="activity-config-container">
        <ContentWrap title="红包兑换活动配置" v-loading="loading">
...
const rules = {
    maximum_daily_redemption_times: [{ required: true, message: '请输入红包兑换最大次数', trigger: 'blur' }],
}
...
const getInfo = async () => {
    loading.value = true
    try {
        const res = await api.edit({ group: 'red_envelope' })
