<template>
    <div class="activity-config-container">
        <ContentWrap title="生涯首充配置" v-loading="loading">
                ...
                <el-form ref="formRef" :model="formData" :rules="rules" label-width="120px" @submit.prevent status-icon class="config-form">
                    ...
                    <el-form-item label="金额配置" prop="amount_list">
                        <div class="amount-list-container">
                            <div class="amount-config-info">
                                <el-alert title="固定三个金额配置" type="info" :closable="false" show-icon />
                            </div>

                            <div v-for="(item, index) in amountListFields" :key="index" class="amount-item">
                                <div class="amount-item-header">
                                    <span class="amount-label">金额配置 {{ index + 1 }}</span>
                                </div>
                                ...
                                <el-input
                                    ...
                                    class="amount-input"
                                    :min="0"
                                    @change="validateAmountItem(index)"
                                >
                                    <template #append>元</template>
                                </el-input>
...
// 类型定义
interface FirstDeposit25Config {
    id: number
    title: string
    context: string
    amount_list: string
...
// API初始化
const api = new baTableApi('/admin/activity.first_deposit_25/')
const formRef = ref<FormInstance>()
const submitting = ref(false)
const loading = ref(false)

// 表单数据
const formData = reactive<FirstDeposit25Config>({
    id: 1,
    title: '生涯首充活动',
    context: '首次充值享受25%奖励',
    amount_list: '[]',
    ...
    amount_list: [
        {
            validator: (rule: any, value: string, callback: any) => {
                if (amountListFields.value.length !== 3) {
                    callback(new Error('必须配置3个金额项'))
                    return
                }
...
// 验证金额配置项
const validateAmountItem = (index: number) => {
    const item = amountListFields.value[index]
    if (item.amount < 0) {
...
// 金额配置操作 - 固定三个配置
const initializeAmountList = () => {
    // 确保始终有3个金额配置
    while (amountListFields.value.length < 3) {
        amountListFields.value.push({
            amount: 100,
            reward_percent: 25,
            recommend: false,
        })
    ...
    // 如果超过3个，则截取前3个
    if (amountListFields.value.length > 3) {
        amountListFields.value = amountListFields.value.slice(0, 3)
    }
}
...
watch(
    ...
    () => {
        try {
            ...
            const parsedArray = objToArray(parsed)

            // 确保有3个配置项
            const defaultItems = [
                { amount: 100, reward_percent: 25, recommend: false },
                { amount: 200, reward_percent: 25, recommend: true },
                { amount: 500, reward_percent: 25, recommend: false },
            ]

            amountListFields.value = defaultItems.map((defaultItem, index) => {
                const parsedItem = parsedArray[index]
                return {
                    amount: Number(parsedItem?.amount) || defaultItem.amount,
                    reward_percent: Number(parsedItem?.reward_percent) || defaultItem.reward_percent,
                    recommend: !!parsedItem?.recommend || defaultItem.recommend,
                }
            })
        } catch {
            // 使用默认的3个配置
    ...
    () => {
        try {
            ...
            if (parsed) {
                ...
                rewardValueConfig.percent = Number(parsed.percent) || 25
            }
        } catch {
            // 使用默认值
        }
...
const getInfo = async () => {
    ...
    try {
        ...
        if (res.code === 1) {
            ...
            formData.title = row.title || '生涯首充活动'
            formData.context = row.context || '首次充值享受25%奖励'
            formData.enable_reward = row.enable_reward ?? 1
            formData.reward_strategy = row.reward_strategy || 'percent'

            // 处理amount_list - 确保有3个固定配置
            const defaultItems = [
                { amount: 100, reward_percent: 25, recommend: false },
                { amount: 200, reward_percent: 25, recommend: true },
                { amount: 500, reward_percent: 25, recommend: false },
            ...
            if (row.amount_list && typeof row.amount_list === 'object') {
                const parsedArray = objToArray(row.amount_list)
                amountListFields.value = defaultItems.map((defaultItem, index) => {
                    const parsedItem = parsedArray[index]
                    return {
                        amount: Number(parsedItem?.amount) || defaultItem.amount,
                ...
                try {
                    ...
                    // 使用默认的3个配置
                    amountListFields.value = defaultItems
                    formData.amount_list = JSON.stringify(amountListFields.value, null, 2)
                }
            }
            ...
            if (row.reward_value && typeof row.reward_value === 'object') {
                ...
                rewardValueConfig.percent = Number(row.reward_value.percent) || 25
                formData.reward_value = JSON.stringify(rewardValueConfig, null, 2)
            } else {
                try {
