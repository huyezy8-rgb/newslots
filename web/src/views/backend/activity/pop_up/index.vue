<template>
    <div class="activity-config-container">
        <ContentWrap title="弹窗活动配置" v-loading="loading">
...
const rules = {
    pop_up_headline: [{ required: true, message: '请输入弹窗活动大标题', trigger: 'blur' }],
    pop_up_title: [{ required: true, message: '请输入弹窗活动小标题', trigger: 'blur' }],
    pop_up_content: [{ required: true, message: '请输入弹窗活动内容', trigger: 'blur' }],
    pop_up_free: [{ required: true, message: '请输入弹窗活动赠送', trigger: 'blur' }],
}
...
const getInfo = async () => {
    loading.value = true
    try {
        const res = await api.edit({ group: 'pop_up' })
