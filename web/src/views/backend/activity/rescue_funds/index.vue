<template>
    <div class="activity-config-container">
        <ContentWrap title="救援金活动配置" v-loading="loading">
...
const rules = {
    rescue_funds: [{ required: true, message: '请输入救援金每日奖励最大值', trigger: 'blur' }],
    rescue_funds_receiving: [{ required: true, message: '请输入救援金领取条件', trigger: 'blur' }],
}
...
const getInfo = async () => {
    loading.value = true
    try {
        const res = await api.edit({ group: 'rescue_funds' })
