...
import { getNewPayRetentionData } from '/@/api/backend/new_pay_retention'
...
const buildParams = (customPage = page.value, customLimit = limit.value) => {
    const params: any = {
        page: customPage,
        limit: customLimit,
    }
...
const fetchData = async () => {
    loading.value = true
    try {
        const res = await getNewPayRetentionData(buildParams())
        if (res.code === 1) {
...
const exportData = async () => {
    ...
    try {
        const res = await getNewPayRetentionData(buildParams(1, total.value || limit.value))
        const rows = res.data.list || []
        const headers = Object.keys(rows[0] || tableData.value[0] || {}).map((key) => ({ label: key === 'date' ? '日期' : key, prop: key }))
        exportRowsToCsv(`new_pay_retention_${dayjs().format('YYYYMMDDHHmmss')}.csv`, headers, rows)
    } finally {
