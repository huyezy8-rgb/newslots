<template>
    <div class="default-main">
        ...
        >
            ...
            <el-table-column prop="new_user" label="首充人数" width="100" />
...
const fetchData = async () => {
    loading.value = true
    try {
        const res = await getLtvData(buildParams())
        if (res.code === 1) {
...
const exportData = async () => {
    ...
    try {
        const res = await getLtvData(buildParams(1, total.value || limit.value))
        const rows = res.data.list || []
        const headers = [
            { label: '日期', prop: 'date' },
            { label: '首充人数', prop: 'new_user' },
            ...Object.keys(rows[0] || tableData.value[0] || {})
                .filter((key) => key !== 'date' && key !== 'new_user')
                .map((key) => ({ label: key, prop: key })),
        ]
        exportRowsToCsv(`first_pay_ltv_${dayjs().format('YYYYMMDDHHmmss')}.csv`, headers, rows)
    } finally {
