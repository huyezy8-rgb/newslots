...
class Upload
{
    ...
    {
        // 检查是否是 ico 文件
        $isIcoFile = in_array($this->fileInfo['suffix'], ['ico']) || in_array($this->fileInfo['type'], ['image/x-icon', 'image/vnd.microsoft.icon']);

        // 检查是否是其他图片格式
        $isOtherImage = in_array($this->fileInfo['type'], ['image/gif', 'image/jpg', 'image/jpeg', 'image/bmp', 'image/png', 'image/webp']) || in_array($this->fileInfo['suffix'], ['gif', 'jpg', 'jpeg', 'bmp', 'png', 'webp']);

        if ($isIcoFile || $isOtherImage) {
            // 对于 ico 文件，跳过 getimagesize 检查，因为某些 ico 文件可能无法正确识别
            if ($isIcoFile) {
                $this->fileInfo['width']  = 16; // 默认 favicon 尺寸
                $this->fileInfo['height'] = 16;
                $this->isImage            = true;
                return true;
            }

            // 对于其他图片格式，使用 getimagesize 检查
