...
class GenerateYesterdayOperationData extends Command
{
    protected function configure()
    {
        $this->setName('generate:yesterday-operation-data')
            ->setDescription('生成前一天所有渠道的运营数据')
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $output->writeln("开始生成前一天的运营数据...");
        $output->writeln("日期: {$yesterday}");
        ...
        $skipCount = 0;
            ...
            foreach ($channels as $channel) {
                ...
                $output->write("处理渠道: {$channelName} (ID: {$channelId})");
        ...
        try {
            // 检查是否已存在数据
                    ...
                    // 计算运营数据
                    $data = $service->getDailyStats($yesterday, $channelId);
                    ...
                    if ($existingData) {
                        ...
                        // 插入新数据
                        OperationData::saveData($yesterday, $data, $channelId);
                        $output->writeln(" - 保存成功");
                    }
            ...
            if ($force) {
                $output->writeln("注意: 使用了 --force 参数，强制重新生成数据");
            }
