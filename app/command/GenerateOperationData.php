...
class GenerateOperationData extends Command
{
    protected function configure()
    {
        $this->setName('generate:operation-data')
            ->setDescription('生成历史运营数据并保存到数据库')
            ->addOption('start-date', 's', \think\console\input\Option::VALUE_REQUIRED, '开始日期 (Y-m-d格式)', date('Y-m-d', strtotime('-30 days')))
            ->addOption('end-date', 'e', \think\console\input\Option::VALUE_REQUIRED, '结束日期 (Y-m-d格式)', date('Y-m-d'))
            ->addOption('channel-id', 'c', \think\console\input\Option::VALUE_OPTIONAL, '渠道ID，不指定表示全部渠道', null)
            ->addOption('force', 'f', \think\console\input\Option::VALUE_NONE, '强制重新生成已存在的数据');
    }
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        if (strtotime($startDate) > strtotime($endDate)) {
            ...
            if (strtotime($endDate) > strtotime(date('Y-m-d'))) {
                throw new \Exception("结束日期不能超过今天");
            }

            // 生成日期列表
            $dateList = $this->generateDateList($startDate, $endDate);
            $output->writeln("需要处理的日期数量: " . count($dateList));
            ...
            if ($channelId) {
                ...
                $channels = OperationDataService::getAllChannelsWithGlobal();
            }
            $service = new OperationDataService();
            $output->writeln("需要处理的渠道数量: " . count($channels));

            $totalCount = count($dateList) * count($channels);
            $successCount = 0;
            $errorCount = 0;
            $skipCount = 0;
            $currentCount = 0;
            ...
            foreach ($dateList as $date) {
                foreach ($channels as $channel) {
                    ...
                    $output->write("进度: {$currentCount}/{$totalCount} - 处理 {$date} 渠道: {$currentChannelName}");
                    ...
                    try {
                        ...
                        if ($existingData && !$force) {
                            $output->writeln(" - 跳过（数据已存在）");
                            $skipCount++;
                            continue;
                        }

                        // 计算运营数据
                        $data = $service->getDailyStats($date, $currentChannelId);
                        ...
                        if ($existingData) {
                            ...
                            // 插入新数据
                            OperationData::saveData($date, $data, $currentChannelId);
                            $output->writeln(" - 保存成功");
                        }
     ...
     * @return array
     */
    private function generateDateList(string $startDate, string $endDate): array
    {
        $dateList = [];
