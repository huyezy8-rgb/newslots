...
class Admin extends Backend
{
    ...
    public function add(): void
    {
        if ($this->request->isPost()) {
            ...
            // 检查渠道权限
            if (isset($data['channel_id']) && !is_null($data['channel_id'])) {
                $this->checkChannelAuth($data['channel_id']);
            }
    ...
    protected function getDataLimitAdminIds(): array
    {
        ...
        // 获取当前管理员的渠道ID
        $currentAdmin = $this->model->find($this->auth->id);
        ...
        // 返回绑定相同渠道的管理员ID列表
        return $this->model
            ->where('channel_id', $currentAdmin->channel_id)
            ->column('id');
    }
     ...
     * @throws Throwable
     */
    private function checkChannelAuth(int $channelId): void
    {
        // 如果是超级管理员，允许操作任何渠道
        if ($this->auth->isSuperAdmin()) {
            return;
        ...
        $currentAdmin = $this->model->find($this->auth->id);
        if (!$currentAdmin) {
            $this->error(__('Current administrator information not found'));
        }

        // 如果当前管理员没有绑定渠道，不允许操作
        if (is_null($currentAdmin->channel_id)) {
            $this->error(__('You have no permission to operate this channel'));
        }

        // 检查是否操作的是自己的渠道
        if ($currentAdmin->channel_id != $channelId) {
            $this->error(__('You can only operate administrators in your own channel'));
        }
    }
}